...
BEGIN
    ...
    JOIN Tracfone.tblTSPTransactionFeed AS ttf WITH (READUNCOMMITTED)
        ON ttf.TSP_ID = dcd.TSP_ID AND dcd.PIN = ttf.TXN_PIN
    JOIN Tracfone.tblTracfoneProduct AS pp WITH (READUNCOMMITTED)
        ON pp.TracfoneProductID = ttf.PRODUCT_SKU AND pp.ProcessBilling = 1
    WHERE
        dcd.FileId = @FileID
        ...
        (
            ...
            WHERE ttf2.Order_No = ttf.Order_No AND ttf2.Processed = 1
    ...
    FROM Tracfone.tblDealerCommissionDetail AS dcd WITH (READUNCOMMITTED)
    JOIN Tracfone.tblTSPTransactionFeed AS ttf WITH (READUNCOMMITTED)
        ON ttf.TSP_ID = dcd.TSP_ID AND dcd.RTR_TXN_REFERENCE1 = ttf.RTR_TXN_REFERENCE1
    JOIN Tracfone.tblTracfoneProduct AS pp WITH (READUNCOMMITTED)
        ON pp.TracfoneProductID = ttf.PRODUCT_SKU AND pp.ProcessBilling = 1
    WHERE
        dcd.FileId = @FileID
    ...
    SELECT
        ...
        CASE
            WHEN LEN(op.TSP_ID) >= 10
                THEN SUBSTRING(op.TSP_ID, 1, 5)
            ELSE op.TSP_ID
        END AS [AccountID],
        o1.Product_ID,
        o.OrderType_ID,
    ...
    JOIN dbo.Order_No AS o WITH (READUNCOMMITTED)
        ON
            o.Order_No = op.Order_No -- original order
            AND o.OrderType_ID IN (22, 23)
    JOIN dbo.Orders AS o1 WITH (READUNCOMMITTED)
        ON
            o1.Order_No = o.Order_No --finds instant spiff of $0  --HERE
            AND ISNULL(o1.ParentItemID, 0) <> 0
            AND o1.Price = 0
    JOIN Products AS p
        ON p.Product_ID = o1.Product_ID
    JOIN Products.tblProductType AS pt
        ON pt.ProductTypeID = p.Product_Type AND pt.ProductTypeID = 4  --SB20240226  spiff item
    JOIN dbo.Orders AS o3 WITH (READUNCOMMITTED)
        ON o3.Order_No = o.Order_No AND ISNULL(o3.ParentItemID, 0) = 0
    LEFT JOIN dbo.Order_No AS o2 WITH (READUNCOMMITTED)
        ON
            o2.AuthNumber = CONVERT(NVARCHAR(50), o.Order_No) --finds spiff order if there is one pending -- noqa
    ...
    JOIN Products.tblProductType AS pt
        ON
            pt.ProductTypeID = p.Product_Type
            AND pt.ProductTypeID = 4  --SB20240226  spiff item
    JOIN dbo.Orders AS o3 WITH (READUNCOMMITTED)
        ON
            o3.Order_No = o.Order_No
    ...
    LEFT JOIN dbo.Order_No AS o2 WITH (READUNCOMMITTED)
        ON
            ...
            AND o2.Process = 0
    WHERE
        CAST(op.Commission_Amount AS DECIMAL(5, 2)) != -1 * o1.Price
        ...
        (
            ...
            WHERE tt.SubjectId = op.Order_No AND tt.Tag = 'Remaining Retro Paid'
        );
    ...
    SELECT
        ...
        op.RemainingSpiff,
        CASE
            WHEN oib.IncentiveOwner != 0
                THEN o2.Order_No--if there's an incentive owner, the spiff gets paid to them                
            ELSE op.RetroSpiffOrder
        END AS RetroSpiffOrder,
        CASE
            WHEN oib.IncentiveOwner != 0
                THEN o3.Price --if there's an incentive owner, the spiff gets paid to them                
            ELSE op.RetroSpiff
        END AS RetroSpiff,
        CASE
            WHEN oib.IncentiveOwner != 0
                THEN o2.Account_ID--if there's an incentive owner, the spiff gets paid to them                
            ELSE op.AccountID
        END AS AccountID,
        op.Product_ID,
        op.OrderType_ID,
    ...
    JOIN dbo.tblOrderItemAddons AS oia WITH (READUNCOMMITTED, INDEX (IX_tblOrderItemAddons_OrderId))
        ON
            ...
            (
                ...
                WHERE
                    f.AddonTypeName IN ('simtype', 'SimBYOPType', 'ESimNumberType')
    ...
    SELECT
        ...
        0 AS RetroSpiff,
        CASE
            WHEN LEN(op2.TSP_ID) >= 10
                THEN SUBSTRING(op2.TSP_ID, 1, 5)
            ELSE op2.TSP_ID
        END AS [AccountID],
    ...
    JOIN dbo.Order_No AS o
        ON o.Order_No = ttf.Order_No AND o.OrderType_ID NOT IN (22, 23);
    ...
    SET
        BundledProductID = x.OrigProductID,
    ...
    JOIN Products.tblXRefilProductMapping AS x
        ON x.OrigProductID = o1.Product_ID AND x.StatusID = 1 AND x.IndIsActive = 1
    ...
    cte AS (
        ...
        JOIN Tracfone.tblTierdiscLog AS tdl
            ON tdl.Logdate < p.Datefilled AND tdl.TierName = p.TierName
    ...
    SELECT DISTINCT
        ...
        CASE
            ...
            WHEN
                ...
                THEN '124315'
            ELSE ISNULL(pot.ProcessAccountID, pot.AccountID)
        END AS [AccountID],
        ProductID,
        orderTypeID,
        void,
    ...
    LEFT JOIN Orders AS o1
        ON o1.Order_No = pot.retroSpiffOrder AND o1.ParentItemID = 0;
    -- noqa: enable=all



    DECLARE
        @PINorRTR VARCHAR(100),
        @orderNo INT,
        @tracSpiff DECIMAL(5, 2),
        @PaidSpiff DECIMAL(5, 2),
        @RetroSpiff DECIMAL(5, 2),
        ...
        BEGIN
            ...
            IF (@tracSpiff <> 0 AND @PinvRTR = 0)
                BEGIN
                    ...
                    SET Processed = 1
                    WHERE Order_No = @orderNo AND TXN_PIN = @PINorRTR;
                END;
            ...
            IF (@tracSpiff <> 0 AND @PinvRTR = 1)
                BEGIN
                    UPDATE Tracfone.tblTSPTransactionFeed
                    SET Processed = 1
                    WHERE Order_No = @orderNo AND RTR_TXN_REFERENCE1 = @PINorRTR;
                END;
            ...
            IF --((
                ...
                --          AND @PaidSpiff = 0 --Instant Spiff
                --          AND @VoidedOrderStatus = 0 --Activation order void status 
                -- AND @AccountID =138380
                -- AND @RetroCStoreAmt =0
                --      )
                --)
                BEGIN
                    ...
                    UPDATE dbo.Account
                    SET
                        AvailableTotalCreditLimit_Amt = AvailableTotalCreditLimit_Amt + @tracSpiff,
                        AvailableDailyCreditLimit_Amt = AvailableDailyCreditLimit_Amt + @tracSpiff
                    WHERE Account_ID = @AccountID;

                    --create spiffdebit
                    EXEC OrderManagment.P_OrderManagment_Build_Full_Order
                        @AccountID = @SpiffDebitAccountID,      -- int
                ...
                BEGIN
                    ...
                    IF (
                        ...
                        BEGIN
                            ...
                            WHERE Order_No = @RetroSpiffOrder AND Paid = 0;

                            UPDATE dbo.Orders
                            SET
                                Dropship_Cost = @RetroSpiff,
                                Price = -1 * @tracSpiff
                            WHERE Order_No = @RetroSpiffOrder;
                            ...
                            FROM dbo.Order_No
                            WHERE
                                Account_ID = @SpiffDebitComAccount
                                AND OrderType_ID = 25
                                AND AuthNumber = CAST(@orderNo AS NVARCHAR(50));

                            SELECT @AgentComm = SUM(Commission_Amt)
                            FROM dbo.Order_Commission
                            WHERE Order_No = @RetroSpiffOrder AND Account_ID <> 2;
            ...
            IF (
                ...
                BEGIN
                    ...
                    UPDATE dbo.Account
                    SET
                        AvailableTotalCreditLimit_Amt = AvailableTotalCreditLimit_Amt + @RemainingSpiff,
